...
anat_name=$(find $orig_dir/$subj/${subj}_${date}*VER1/ -name *_MPRAGE_ADNI*_2.98*)
echo $anat_name &>> $results_dir/$subj/check/log_x1.txt
anat_arr=($anat_name)
echo "${anat_arr[0]}" > $results_dir/$subj/check/images_used_anat.txt
#get the scan number of anat 
scanum=$(sed 's/.*S\(.*\)_MPRAGE.*/\1/' $results_dir/$subj/check/images_used_anat.txt)
echo $scanum &>> $results_dir/$subj/check/log_x1.txt

echo " -copying & converting T1 anat images"
echo "-----------------------------------------"
dictov -in $dicmdir -scans $scanum -out tmp.v &>> $results_dir/$subj/check/log_x1.txt 
visotrop tmp.v -reso 1 -out ${subj}_mr_t1_or.v &>> $results_dir/$subj/check/log_x1.txt
rm -f tmp.v

echo " -converting DICOM images for DWI"
echo "-----------------------------------------"
